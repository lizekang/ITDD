...
class Translator(object):
    ...
    def _translate_batch(self, batch, data):
        ...
        src, knl, enc_states, his_memory_bank, src_memory_bank, knl_memory_bank, src_lengths = self._run_encoder(
            batch, data_type)
        self.model.decoder.init_state(knl[400:600, :, :], src[100:, :, :], src_memory_bank, enc_states)
        ...
        if "tgt" in batch.__dict__:
            results["gold_score"] = self._score_target(
                batch, his_memory_bank, src_memory_bank, knl_memory_bank, knl, src_lengths, data, batch.src_map
        ...
        for i in range(self.max_length):
            ...
            dec_out, dec_attn = self.model.decoder(
                inp, his_memory_bank, src_memory_bank, memory_lengths=memory_lengths, step=i
            )
        ...
        emb, decode1_memory_bank, decode1_mask = self.model.encoder.histransformer(first_dec_words[:50, :, :], None)
        self.model.decoder2.init_state(first_dec_words[:50, :, :], knl[400:600, :, :], None, None)
    ...
    def _score_target(self, batch, his_memory_bank, src_memory_bank, knl_memory_bank, knl, src_lengths, data, src_map):
        ...
        first_dec_out, first_attns = self.model.decoder(tgt_in, his_memory_bank, src_memory_bank,
                                                        memory_lengths=None)
